...
\ifchinese
要想获得 \FlightGear{} 的追踪信息，一下命令也许会帮到你（可能需要针对一些操作系统做修改，也许其他系统不能工作）：
\fi
\iffalse
\IfLanguageName{english}{
...
仔细检查 \texttt{configure} 时的错误信息，很多情况下它会告诉你缺少了什么。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
\item{缺少\Index{权限}}\\
如果你使用 4.0 发布版之前的 \Index{XFree86}，\FlightGear{} 的二进制文件可能需要 setuid 到 root 以获得访问某些基于 3DFX 图形加速卡的权限（或者如前文所述的特定的内核模块）。
你可以用以下两种方法之一：
...
\ifchinese
\item{可执行文件拒绝运行}\\
也许你需要直接在 Windows 资源管理器里双击 \texttt{fgfs.exe} 文件，或者通过 MS-DOS 命令行运行。如果在资源管理器里双击也不管用（除非你在 \texttt{autoexec.bat} 或其他地方修改了 \texttt{FG\_ROOT} 环境变量），可以双击 \texttt{fgrun}。更多相关信息可以参考第~\ref{prefligh} 章。
...
默认情况下，\FlightGear{} 使用 GNU GCC 来构建。在 Win32 下移植的 GNU GCC 时 \Index{Cygwin}。有关 \textit{Makefiles} 对 MSVC 或 MSC DevStudio 的需求可以看：
\fi
\iffalse
\IfLanguageName{english}{
