...
\ifchinese
想获取这些的代码，可以看这个介绍如何获取 \FlightGear{} 的代码仓库
\fi
%\IfLanguageName{english}{
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ifchinese
历史上，\FlightGear{} 是遗传自 LaRCsim 并加入了 Navion 飞机。这带来了很多限制（最重要的是很多特性是硬编码实现的，而不是配置文件），多次尝试开发或者加入额外的\Index{飞行模型（flightmodel）}。结果就是，\FlightGear{} 支持多种不同的飞行模型，可以在运行时选择。
\fi
%\IfLanguageName{english}{
...
\ifchinese
\item Andrew Ross 创建了另一个飞行模型 \textit{Yet Another Simulator}(\YASim{}\index{YASim})。\YASim{} 使用和其他飞行动态模型（FDM）完全不同的方式，基于空气动力学模型而不是基于几何信息。\YASim{} 有特定的高级直升机 FDM。
\fi
%\IfLanguageName{english}{
...
\ifchinese
\item 最终，有了 \Index{UIUC 飞行模型}，由伊利诺伊大学厄巴纳-香槟分校的团队研发。这个项目最开始是面向飞行器结冰\index{icing 结冰!modelling 建模}建模，而现在则包括“非线性”空气动力学，也就是让飞机在极端姿态下变得更真实，比如失速和大迎角飞行。用来演示这些能力的两个好例子是 \Index{Airwave Xtreme 150} \Index{滑翔机} 和1903年\Index{莱特兄弟的“飞行者”}。更多有关 UIUC 飞行模型的细节可以参考
\fi
%\IfLanguageName{english}{
...
\end{itemize}
\ifchinese
甚至可以在 FlightGear 的场景里使用外部 FDM \index{FDM!外部}运行在不同的计算机或通过本地计算机的具名管道\index{FDM!管道}——尽管对刚刚接触 \FlightGear{} 的人来说也许是非建议安装的。
\fi
%\IfLanguageName{english}{
...
还有一点，仅此一点，本手册中出现的材料仅仅在此出现。正如蒙田所说我们“只是集中别人手里的大把花束，没有装饰我自己，只是用带子将她们捆在了一起”。大多数（很不幸不是全部）出现在这里的信息也可以从 \FlightGear{}\index{FlightGear 网站}网站获取
\fi
...
\ifchinese
\textit{\FlightGear{} 手册}是完善全部 \FlightGear{} 文档\index{FlightGear 文档}的第一步。目标读者和终端用户是那些没有兴趣了解 \Index{OpenGL} 内部机制或构建地景的人。我们希望有朝一日会有随同的 \textit{\FlightGear{} 编程指南}\index{FlightGear 编程指南} 和 \textit{\FlightGear{} 地景设计指南}\index{FlightGear 地景设计指南}，用来描述现在软件包名是 \TerraGear{} 的地景工具；还有一个 \textit{\FlightGear{} 飞行学校}\index{FlightGear 飞行学校}的软件包。
\medskip
