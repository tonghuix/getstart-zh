...
你也许已经用过 \Index{Microsoft}{\copyright} Flight Simulator（微软模拟飞行） 或者其他商业级 PC 飞行模拟器。这些模拟器大多 50 美元左右，买一个并不是什么大问题，然而为了运行这些飞行模拟器则需要电脑超强的硬件性能，而这些大概需要 1500 美元，尽管硬件价格持续下跌。
...
\ifchinese
\fi
\item 所有商业飞行模拟器都有一下弊病：由一群开发者根据自身的私有需要来定义什么对他们是重要的，并限制终端用户的接口。尝试联系一个商业开发者让他听你的声音，这在那种情况下是一个巨大的挑战。然而 \FlightGear{} 是为所有人设计的，所有的一切都是开放的。
%\IfLanguageName{english}{
...
\ifchinese
\item 商业模拟器通常要在实用性和特性之间妥协。大多数商业开发者希望服务于更广泛的用户群，包括严肃的飞行员、初学者，甚至是休闲玩家。
而事实上，最终都会向期限和资金妥协。由于 \FlightGear{} 是自由且开放，也就没有了这些妥协。不会有发行商扼住我们的脖子，我们都是志愿者大家，一起制定自己的发行期限。我们也支持那些商业开发者没兴趣的领域，比如科研院所。
\fi
%\IfLanguageName{english}{
...
\begin{itemize}
\ifchinese
\item \textbf{民用航空：}\index{Flight simulator!civilian} 这个项目主要针对民用航空模拟。它能够适应通用航空和民用客机。我们的长期想法是希望 \FlightGear{} 能成为 FAA 许可的飞行训练设备。对某些用户来说会比较失望的是，目前并不是一个空战模拟器；不过相应的特性并非明确排除在外。我们只是没有开发者对空战模拟的系统性需求有严肃的兴趣。
\fi
%\IfLanguageName{english}{
...
\item\textbf{跨平台：}\index{Flight simulator!multi-platform}开发者尝试让代码尽可能做到平台无关性。根据我们的观察，对飞行模拟感兴趣的人运行了各种计算机硬件和操作系统，本项目代码支持如下\Index{操作系统}：
 \begin{itemize}
  ...
  \item\Index{Windows NT/2000/XP/Seven} (Intel/AMD 平台),
...
\ifchinese
 ...
 \Index{GPL} 也经常被误解。简单来说 GPL 授权你可以自由的拷贝和再发布程序。你可以修改或者用修改以后和原始的程序谋利，然后当再发布软件的时候，必须保证再发布的软件源代码与原始的使用同样授权许可。一言以蔽之：
\medskip
...
\ifchinese
\item\textbf{用户支持及用户可扩展性：}\index{Flight simulator!user-supported} \index{Flight simulator!user-extensible} 不像大多数商业模拟器，\FlightGear{} 的地景和飞行器格式，内部变量，API，以及其他所有都是用户可以访问的，并有从初学者开始的文档。即使没有任何明确的开发\Index{文档}（自然的肯定以后会写），任何人都可以参考\Index{源代码}来学习其背后原理。开发者的目标是建构一个基础的引擎，它可以让地景设计者、仪表板工程师、专家或者 ATC 系统设计者，声音艺术师、以及其他人共建其中。我们希望这个项目，包括开发者和终端用户，都能从全世界数百“模拟器达人”的创造性想法中获益。
\fi
%\IfLanguageName{english}{
...
\end{itemize}
\ifchinese
毋庸置疑，\Index{Linux} 项目的成功，肇始于 Linus Torvalds \index{Torvalds, Linus} 并影响了无数的开发者。并不仅限于 \Index{Linux} 显示出通过互联网分布式开发尖端软件是可能的，同时也证明这样的成果可以超越同等质量的商业产品。
\fi
%\IfLanguageName{english}{
...
\smallskip
 \noindent
\ifchinese
图 1：UNIX 系统下的 \FlightGear{}：\textit{旧金山国际机场跑道前糟糕的进近——来自本手册的一位作者截图\ldots}
\fi
%\IfLanguageName{english}{
