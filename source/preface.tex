...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{free} 章，~\nameref{free}，介绍 \FlightGear{}，并讲解其中的背景知识和哲学，还有系统要求。
\fi
\IfLanguageName{french}{
...
filosofia di base che c'\`{e} dietro al programma e descrive i requisiti di sistema
 ...
 \noindent
\ifchinese
第~\ref{prefligh} 章，~\nameref{prefligh}，这里将提供如何安装二进制发行版\index{binary distribution 二进制发行版}和附加地景及飞行器的介绍。
\fi
\IfLanguageName{french}{
...
\medskip
 ...
 \noindent
\ifchinese
 第~\ref{takeoff} 章，~\nameref{takeoff}，描述了如何实际启动已经安装的程序。
 还包括概述一些命令行参数和配置文件。
\fi
\IfLanguageName{french}{
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{features} 章，~\nameref{features}，描述了一些 \FlightGear{} 中特别为高级用户提供的特性。
\fi
...
\textbf{Partie III : Tutorial}
 ...
 \noindent
\ifchinese
第~\ref{tutorials} 章，~\nameref{tutorials}，提供了为新飞行员准备的一些教程。
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{basic} 章，~\nameref{basic}，提供了一份很基础的飞行教程举例讲解在 \FlightGear{} 中如何飞行，并配以丰富的飞机仪表、按钮及其他设备的插图 
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{crosscountry} 章，~\nameref{crosscountry}，从非默认安装旧金山地区出发，做一次短途转场飞行，
\fi
%\IfLanguageName{english}{
...
eseguito con l'installazione di default
 ...
 \noindent
\ifchinese
第~\ref{IFR Tutorial} 章，~\nameref{IFR Tutorial}，讲述如何使用仪表飞行规则（IFR），在云雾中完成短途转场飞行。
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
附录~\ref{missed}，~\nameref{missed}，我们尝试帮助你解决一些使用 \FlightGear{} 时的常见问题。
\fi
%\IfLanguageName{english}{
...
l'utilizzo di FlightGear
 ...
 \noindent
\ifchinese
最后的附录~\ref{landing}，~\nameref{landing}，我们会特别提到 \FlightGear{} 项目的贡献者，同时也想给那些希望贡献到 \FlightGear{} 开发中的人一些信息并告诉大家还有哪些有待完成。
\fi
%\IfLanguageName{english}{
...
\item 最后就是 \FlightGear{} 的\textbf{官方论坛}\index{forum 论坛} \web{https://forum.flightgear.org/}.
\fi
%\IfLanguageName{english}{
