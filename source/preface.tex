...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ifchinese
\FlightGear{} 是一个自由的飞行模拟器，由互联网上一群对飞行模拟和编程狂热爱好的人开发。这本《FlightGear 手册》希望介绍给初学者如何快速上手 \FlightGear{}，并享受飞行的乐趣。此处不会提供对 \FlightGear{} 全部特性和插件详细介绍的文档，只是让新手了解如何开始和探索 \FlightGear{} 所提供的各种体验。 
\fi
...
\ifchinese
此版本的文档是为 \FlightGear{} \version{} 所写。使用早期版本 \FlightGear{} 的用户也能在此文档中找到有用的信息，然而本文中提到的很多新特性，可能无法使用。
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{free} 章，想自由的飞翔？玩 {\FlightGear{}} 吧！介绍 \FlightGear{}，并讲解其中的背景知识和哲学，还有系统要求。
\fi
\IfLanguageName{french}{
...
filosofia di base che c'\`{e} dietro al programma e descrive i requisiti di sistema
 ...
 \noindent
\ifchinese
第~\ref{prefligh} 章，飞行前：安装 \FlightGear{}。这里将提供如何安装二进制发行版\index{binary distribution 二进制发行版}和附加地景及航空器的介绍。
\fi
\IfLanguageName{french}{
...
\medskip
 ...
 \noindent
\ifchinese
 第~\ref{takeoff} 章，起飞：如何启动程序。描述了如何实际启动已经安装的程序。
 还包括概述一些命令行参数和配置文件。
\fi
\IfLanguageName{french}{
...
opzioni della riga di comando e sui file di configurazione
 ...
 \noindent
\ifchinese
第~\ref{flight} 章，飞行中：仪表、键位和菜单。描述了如何操作这个程序，
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{features} 章，特性。描述了一些 \FlightGear{} 中特别为高级用户提供的特性。
\fi
...
\textbf{Partie III : Tutorial}
 ...
 \noindent
\ifchinese
第~\ref{tutorials} 章，教程。提供了为新飞行员准备的一些教程。
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{basic} 章，基础飞行模拟教程。提供了一份很基础的飞行教程举例讲解在 \FlightGear{} 中如何飞行，并配以丰富的飞机仪表、按钮及其他设备的插图 
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
第~\ref{crosscountry} 章，短途转场飞行教程。从非默认安装的旧金山地区出发，做一次短途转场飞行，
\fi
%\IfLanguageName{english}{
...
eseguito con l'installazione di default
 ...
 \noindent
\ifchinese
第~\ref{IFR Tutorial} 章，IFR 转场飞行教程。讲述如何使用仪表飞行规则（IFR），在云雾中完成短途转场飞行。
\fi
%\IfLanguageName{english}{
...
\medskip
 ...
 \noindent
\ifchinese
附录~\ref{missed}，复飞：如果有什么拒绝工作。我们尝试帮助你解决一些使用 \FlightGear{} 时的常见问题。
\fi
%\IfLanguageName{english}{
...
l'utilizzo di FlightGear
 ...
 \noindent
\ifchinese
最后的附录~\ref{landing}，落地：离开飞机之前的一些想法。我们会特别提到 \FlightGear{} 项目的贡献者，同时也想给那些希望贡献到 \FlightGear{} 开发中的人一些信息并告诉大家还有哪些有待完成。
\fi
%\IfLanguageName{english}{
...
\ifchinese
如果你使用 Linux，会发现 \FlightGear{} 已经在你的发行版软件包列表里了。
...
\begin{itemize}
\ifchinese
\item 一张手边备查的\textbf{快速参考}\index{leaflet 快速参考}可以在基础包的 \texttt{/FlightGear/Docs} 目录下找到并打印下来，也可以从这里下载
\fi
%\IfLanguageName{english}{
...
%}{}
\IfLanguageName{french}{
 \item un petit \textbf{d\'{e}pliant}\index{leaflet 快速参考} \`{a} imprimer. Bien pratique en vol, il peut \^{e}tre trouv\'{e} dans le paquetage de
...
\ifchinese
\item 额外有关某些特性和功能的\textbf{用户文档}可以在基础包的 \texttt{/Docs} 目录找到。
\fi
%IfLanguageName{english}{
...
\ifchinese
\item 还有一个官方的 \FlightGear{} \textbf{维基}\index{wiki}\index{维基}，可以前往 \web{http://wiki.flightgear.org}。
\item 最后就是 \FlightGear{} 的\textbf{官方论坛}\index{forum}\index{论坛} \web{https://forum.flightgear.org/}.
\fi
%\IfLanguageName{english}{
