...
(typically something like \texttt{c:$\backslash$Program Files$\backslash$FlightGear$\backslash$Win32$\backslash$bin}), set the \Index{environment variables} by typing 
\medskip
...
Of course, you can create a batch file with a Windows text editor (like notepad) using the
lines above.
...
the top \texttt{FG\_ROOT} directory (e.g. \texttt{c:$\backslash$Program Files$\backslash$FlightGear}). 
\end{itemize}
