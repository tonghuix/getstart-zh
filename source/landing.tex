...
Ces donn\'{e}es de terrain sont disponibles respectivement \`{a} partir de :
}{}
...
\iflanguage{french}{
 \item Apr\`{e}s avoir am\'{e}lior\'{e} la prise en charge des \Index{sc\`{e}nes} et des \Index{textures}, le \index{taux de rafra\^{i}chissement} de l'image diminua \`{a} tel point qu'il \'{e}tait impossible de faire voler un avion dans \FlightGear{} au printemps 1998. Cette difficult\'{e} fut contourn\'{e} en exploitant la prise en charge mat\'{e}rielle d'\Index{OpenGL}, qui devint disponible \`{a} cette date, et en impl\'{e}mentant la \Index{vue frustrum culling} (une technique de rendu qui ignore la
 partie du paysage non visible d'une sc\`{e}ne), r'{e}alis'{e} par Curt Olson\index{Olson, Curt}.
