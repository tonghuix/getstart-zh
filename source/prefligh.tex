...
注意不要解压缩 tarball 包里像 958402.gz 这样以数字命名的地景文件——这将会由 \FlightGear{} 在飞行时自动解压。

一旦你解压了 tarball，\texttt{Terrain} 和 \texttt{Objects} 目录会包含额外的子目录，里面有你新加的地景。

要使用新的地景，只需要选择地景所在区域的机场。如果你在使用 \FlightGear{} 启动器（FlightGear Launcher），在选择机场前请先按刷新按钮。
\fi
%\IfLanguageName{english}{
...
在 \FlightGear{} 里，到 Environment（环境）菜单下选择 Scenery Download（地景下载）选项。然后选择上面创建的目录并启用 automatic scenery（自动地景下载）。
...
\ifchinese
\FlightGear{} 基本包里只有少数的飞行器可以在 \FlightGear 里面玩。开发者已经创建了大量的飞行器，从二战时的“喷火”（Spitfile）战斗机到现在的大型旅客客机比如波音747。
...
\ifchinese
只需要将文件解压缩你安装位置的 \texttt{data/Aircraft} 子目录下即可。飞行时以 \texttt{.zip} 格式下载。解压缩以后，将会在 \texttt{data/Aircraft} 目录下有一个包含此飞行器的子目录。下次你运行 \FlightGear{}，新的飞行器就可以用了。

在 Mac OS X，可以使用图形化启动器来安装下载的飞行器文件，可以参考\ref{sceneryOnMac}。
\fi
%\IfLanguageName{english}{
...
\ifchinese
上面提到的软件包都会包括安装 PDF 版的\textit{《FlightGear 手册》}，这样可以通过 Adobe Reader 打印下来。下载 Adobe Reader: \footnote译者注：在 Linux 下可以安装自由开源的 Evince 软件 \web{https://wiki.gnome.org/Apps/Evince}
\medskip
