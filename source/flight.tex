...
The status of your engines when you begin the simulation depends on the way you initiated your session.  If the \index{engine}s are off, you'll have to start them before proceeding any further.  For that you'll need the \index{ignition switch}, which you'll find in the lower-left corner of the instrument panel. It is shown in Fig.\,4.
...
addition, you should advance the \Index{throttle} to about 25\,\%.

Now you can hit the ``s'' key to operate the \index{starter}. When you do, you'll notice that the ignition switch moves to the ``START'' position.  The engine will start after a few seconds.
Afterwards you can bring the throttle back to idle (all the way out).

In addition, you should check to see whether the \Index{parking brake}s are engaged (red
...
of the plane during flight via the keyboard you should ensure that, (i)  \texttt{\Index{NumLock}}
...
or you aren't sure, just click anywhere in the \FlightGear{} window).  With the \texttt{NumLock} active, the following main \Index{keyboard controls}
for controlling the aircraft should work:
 \eject

\noindent
 Tab.\,2: Main \Index{keyboard controls} for \FlightGear{} on the
...
\noindent
 Tab.\,3: View directions\index{view directions}
...
When the \Index{autopilot} is enabled, some of the numeric keypad keys function differently:
...
shot\index{screenshot} as \\ \texttt{fgfs-screen-XXX.ppm} to
...
indicates
whether the \Index{rudder} and \Index{aileron}s are co-ordinated. During turns, you always
have to
...
To the right-hand side of the artificial horizon you will find the \Index{altimeter}
showing the height
...
indicator\index{RPM indicator}, which displays the rotations per minute  in hundreds. The
...
Pressing Ctrl-C highlights the clickable hotspots.
...
controls the rudder  and forwards/backwards controls the
throttle.
...
useful for looking around the cockpit, or out a side window.
...
\section{Exiting the simulator}\index{exit}\index{exiting}
