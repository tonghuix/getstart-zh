...
\^{e}tre gr\^{a}ce \`{a} de l'exp\'{e}rience acquise sur d'autres simulateurs. Si vous \^{e}tes un vrai
...
\iflanguage{french}{
Suivant le type d'a\'{e}ronef, vous pouvez avoir \`{a} démarrer le(s) moteur(s) avant
...
\iflanguage{french}{
Pour les a\'{e}ronefs disposant d'un moteur \`{a} piston, les magn\'{e}tos sont control\'{e}s par les
...
`\ ̃' pour s\'{e}lectionner tous les moteurs \`{a} la fois. La plupart des magn\'{e}tos ont 4 positions - OFF, LEFT, RIGHT et BOTH.
...
avec la souris, comme d\'{e}crit ci-dessous. Mais,  quelle que a la fa\c{c}on dont vous contr\^{o}lez votre
a\'{e}ronef, vous serez oblig\'{e} d'utiliser le clavier, ne serait-ce que pour quelques actions.

L'attribution de fonctions \'{a} des touches (\index|raccourcis claviers!configuration) ne sont pas cod\'{e}s en durs,
mais ajustables par l'utilisateur. Vous pouvez contr\^{o}ler et modifier ces param\'{e}tres via le fichier \texttt{keyboard.xml},
qui peut \^{e}tre trouv\'{e} \`{a} dans le r\'{e}pertoire  principal de \FlightGear{}. C'est un fichier texte au format
...
pilotes automatiques sp\'{e}cifiques \`{a} un a\'{e}ronef en partiulier et qui sont contr\^{o}l\'{e}s \`{a} partir du cockpit.
...
\iflanguage{french}{
De m\^{e} qu'elle permet de s\'{e}lectionner des \'{e}l\'{e}ments du menu ou de cliquer sur des commandes au sein du cockpit,
votre souris peut \'{e}galement \^{e}tre utilis\'{e}e dans \FlightGear{} \`{a} de nombreuses fins utiles.

Il y a trois \Index{modes souris} : Normal (par d\'{e}faut), Contr\^{o}l et Vue. Vous pouvez basculer de l'un \`{a} l'autre
...
\iflanguage{french}{
En mode Normal, vous pouvez contr\^{o}ler les menus et les commandes du tableau de bord. Ce mode est
indiqu\'{e} par un curseur disposant d'une fl\`{e}che normale.
...
gauche de celui-ci pour faire d\'{e}cro\^{i}tre la valeur, et sur le c\^{o}t\'{e} droit pour l'augmenter. Cliquez avec le
...
L'activation de ce mode est indiqu\'{e} par la pr\'{e}sence d'un curseur de souris en forme de croix.
...
Maintenir le bouton gauche appuy\'{e} tout en bougeant la souris vers la droite ou la gauche contrôle le
...
Enfin, la molette peut \^{e}tre utilis\'{e} pour param\'{e}trer le trim.

Ce mode est particulièrement pratique si vous n'avez pas de joystick. On a ainsi un
bien meilleur contr\^{o}le de l'a\'{e}ronef qu'avec le clavier. Si vous avez l'intention de piloter
r\'{e}gulièrement votre avion avec la souris, il vous est recommand\'{e} d'activer l'auto-
...
est indiqu\'{e} par la pr\'{e}sence d'un curseur de souris \`{a} double flèche.
...
On peut utiliser la molette pour zoomer ou dé-zoomer. Cliquer sur le bouton gauche de la souris rempositionne
la vue à sa position initiale, en g\'{e}n\'{e}ral droit devant.
...
\iflanguage{french}{
 ...
 \item \textbf{R\'{e}initialiser (Shift-Esc)} r\'{e}initialise \index{r\'{e}initialiser un vol} le vol \`{a} la position de d\'{e}marrage s\'{e}lectionn\'{e}e. Tr\`{e}s pratique si jamais vous \^{e}tes perdu ou que quelque chose tourne mal.
  ...
  \item \textbf{R\'{e}pertoire des captures} vous permet de pr\'{e}ciser le r\'{e}pertoire o\`{u} les captured d'\'{e}cran seront  sauvegard\'{e}es.
...
\item \textbf{Affichage}\index{Affichage}
 \begin{itemize}
  ...
  \item \textbf{Options de rendu} configure diverses fonctionnalit\'{e}s graphiques. Ceci vous permet de g\'{e}rer le compromis entre les effets visuels sympathiques, comme les ombres, les nuages 3D et les r\'{e}flexions sp\'{e}culaires et le taux de rafra\^{i}chissement des images. Pour vous permettre d'arriver \`{a} un compromis acceptable, l'activation de l'option ``Afficher le taux de rafra\^{i}chissement'' option dans le menu Options d'affichage, vous permettra de visualiser le taux d'images par seconde (\textit{frames per second, fps}) actuel dans l'angle inf\'{e}rieur droit de l'\'{e}cran. La plupart des personnes trouvent qu'un taux d'environ 20 fps est suffisant pour voler. Ce taux est d\'{e}pendant des fonctionnalit\'{e}s graphiques que vous avez activ\'{e}, la visibilit\'{e} du moment (param\'{e}tr\'{e} par Z/z), le nombre d'objets visibles et leur niveau de d\'{e}tails (\textir{Level Of Detail, LOD}).
  \item \textbf{Options de la vue} configure quelles vues sont activ\'{e}es.
  \item \textbf{Options d'affichage du cockpit} configures la vue au sein du cockpit, le mouvement de la t\^{e}te du pilote, le voile noir d\^{u} \`{a} la prise importante de G et le voile rouge d\^{u} \`{a} la prise important de G n\'{e}gatifs.
  \item \textbf{Ajuster le niveau de d\'{e}tail} fixe la distance jusqu'\`{a} laqelle diff\'{e}rents niveaux de d\'{e}tails sont affich\'{e}s. Ceci affecte les textures et objets affich\'{e}s par le simulateur.
  \item \textbf{Ajuster la position de la vue} Affiche le d\'{e}calage de vue actuel. Vous pouvez ajuster cette valeur en d\'{e}pla\c{c}ant les contr\^{o}les. Sinon, vous pouvez r\'{e}aliser de petits ajustements \`{a} votre point de vue en utilisant la souris (voir ci-dessus).
 ...
 \item \textbf{(D\'{e}s)Activer le tunnel ILS} (d\'{e}s)active un tunnel virtuel ou vous guider jusqu'\`{a} la piste sur un plan d'approche normal. Utile si vous avez des difficult\'{e}s a trouver le bon plan d'approche pour atterrir.
  \item \textbf{Rejeu instantan\'{e} (Ctrl-R)} contr\^{o}le la fonctionnalité de rejeu instantan\'{e} - un bon outil pour v\'{e}rifier la qualité\'{e} de vos atterrissages !
   \item \textbf{Options de vue st\'{e}r\'{e}oscopique} configure l'affichage st\'{e}r\'{e}oscopic (en relief 3D), en utilisant des lunettes Rouge/Vert ou d'autres m\'{e}thodes d'affichage.
\end{itemize}
}{}
...
\item \textbf{Position}\index{position}
 \begin{itemize}
   \item \textbf{Positionner l'a\'{e}ronef au sol} positionne l'a\'{e}eronef sur la piste de n'importe quel a\'{e}roport install\'{e}. Vous avez besoin de conna\^{i}tre le code OACI de l'a\'{e}roport \`{a} partir duquel vous souhaitez d\'{e}marrer (par exemple KSFO pour San Fransisco International).
   ...
   \item \textbf{Attitude al\'{e}atoire} Place l'a\'{e}ronef avec une attitude, un cao et une vitesse al\'{e}atoires. Utile pour s'entra\^{i}ner \`{a} la r\'{e}cup\'{e}ration depuis des attitudes inhabituelles.
...
\iflanguage{french}{
   ...
   Vous pouvez param\'{e}trer le pilote automatique de diff\'{e}rentes mani\`{e}re - du simple maintien du niveau des ailes
   au suivi d'un ILS.
...
\iflanguage{french}{
  ...
  la m\'{e}t\'{e}o telle qu'elle est diffus\'{e}e par la station m\'{e}t\'{e}o la plus proche (g\'{e}n\'{e}ralament un a\'{e}roport) via les
  ...
  \item \textbf{Param\`{e}tres d'incendie} d\'{e}finit si le crash d'un avion au sol provoque un feu de for\^{e}t r\'{e}aliste, qui
...
\iflanguage{french}{
  ...
  \item \textbf{Param\`{e}tres GPS} configure les points de cheminement et viesualise les informations de route pour le GPS.
  ...
  \item \textbf{Pannes des instruments} configures la panne d'un instrument sp\'{e}cifique de l'a\'{e}ronef.
}{}
 \end{itemize}
...
\item \textbf{IA}\index{IA}
 \begin{itemize}
  ...
  Notez que cette configuration ne prendra effet que lors du prochain d\'{e}marrage du simulateur.
  \item \textbf{Contr\^{o}le des ailiers} vous permet, suivant les a\'{e}ronefs, de contr\^{o}ler vos ailiers IA.
  \item \textbf{Contr\^{o}le du ravitailleur} vous permet de cr\'{e}er dynamiquement un avions de ravitallement en vol, si
...
\item \textbf{Multijoueurs}\index{multijoueurs}
 \begin{itemize}
  ...
  \item \textbf{Fen\^{e}tre de clavardage} vous permet de discuter avec un autre a\'{e}ronef dans l'environnement multi-joueurs.
  ...
  de contr\^{o}le a\'{e}rien dans l'environnement multi-joueurs. Certains menus contiennent des sous-menus d'options.
  \item \textbf{Liste des pilotes} affiche une liste des autres pilotes multi-joueurs \`{a} port\'{e}e, avec leur distance, rel\`{e}vement et
  altitude.
...
Au centre, vous trouvez l'indicateur d'assiette \Index{assiette} (en degr\'{e}s) avec l'\Index{indicateur des ailerons} au-dessus
