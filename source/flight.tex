...
当你启动发动机以后，你需要检查\Index{停留刹车}是不是开启，如果开启可以按“B”键释放刹车。
\fi
\iffalse
\IfLanguageName{english}{
...
要旋转一个无线电频率旋钮或者线性控制，比如油门杆，点击左侧来减少数值，点击右侧来增加数值。点鼠标左键可做小幅度调整，点鼠标中键可做大幅调整。有些控制，比如径向也可以使用鼠标滚轮，而另一些比如油门，支持鼠标左键拖拽。
...
当你没有游戏杆的时候使用此模式非常有用，可以提供比键盘更好的飞行器控制。如果你打算经常使用鼠标控制飞行器，建议打开自动协调选项，这样副翼和方向舵会一致控制。可以在启动模拟器时指定 \texttt{-$ $-enable-auto-coordination} 选项来启用这个功能，或者在启动器里勾选自动协调（auto-coordination）选项。
\fi
\iffalse
\IfLanguageName{english}{
...
移动鼠标可以平移和倾斜当前位置的视图。此模式非常有用，可以在驾驶舱里方便观察，或者查看侧面的窗户。鼠标滚轮可以控制放大和缩小。点左键可以回到初始位置，通常是平视前方。

按住中键并移动鼠标可以让你移动视点本身，向左向右向上向下。按住CTRL和鼠标中键并移动鼠标，可以视点前后移动。
\fi
\iffalse
\IfLanguageName{english}{
...
\item \textbf{File（文件）}
\begin{itemize}
  \item \textbf{Reset (Shift-Esc)}（重置） 重置\index{重置飞行}到选定的起始位置。可以让你在发生问题的时候快速上手。
  ...
  \item \textbf{Input Configuration} 配置鼠标的腥味。
...
\ifchinese
\item \textbf{Autopilot}\index{自动驾驶} 此菜单仅在飞行器默认配置自动驾驶时可用。一些飞机的自动驾驶是通过其仪表板来配置的，在这种情况下此菜单不可用。
\fi
\iffalse
\IfLanguageName{english}{
...
\item \textbf{Route Manager} 设置自动导航的航路（导航点）列表。导航点可以是机场也可以是航路点，航向、距离和前往当前导航点的时间会在 HUD 上显示。
\item \textbf{Previous Waypoint} 从航路列表上选择前一个导航点。
\item \textbf{Next Waypoint} 从航路列表上选择下一个导航点。
 \end{itemize}
\fi
\iffalse
\IfLanguageName{english}{
...
\item \textbf{Ambiente}\index{ambiente}
 ...
 \begin{itemize}
\ifchinese
\item \textbf{Weather} 可以设置当前天气，选择天气场景，或者使用最近的气象情报点（通常是机场）的 METAR 天气报告。你可以使用基本的天气，也就是会明确设置天气状况；或者使用高级天气，模拟更大的天气系统，会牺牲本地的精确性。
\item \textbf{Environment Settings} 配置季节材质贴图（夏季或冬季），以及地面状态，比如雪线和扬尘。
\item \textbf{Time Settings} 允许你设置模拟器的当前时间，加速模拟，以及改变模拟器的时间变化率。会显示 UTC 时间和本地时间。
\item \textbf{Wildfire Settings} 配置飞机坠毁时产生真实的火焰效果，这可以波及并可以用飞机上适当的灭火瓶来扑灭。
\end{itemize}
\fi
\iffalse
\IfLanguageName{english}{
...
\begin{itemize}
  ...
  \item \textbf{System Failures} 配置随机的飞行器系统失效，比如真空系统。
   \item \textbf{Instrument Failures} 配置特定飞机上，随机的仪表失效。
  \end{itemize}  
\fi
\iffalse
\IfLanguageName{english}{
...
最左侧边缘有两个标尺：里侧表示\Index{速度}（以节为单位）外侧表示\Index{油门}位置。最右侧边缘的标尺表示\Index{高度}——左侧的表示相对地面的高，而右侧表示相对海平面高度，两者都以英尺为单位。
