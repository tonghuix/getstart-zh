...
\ifchinese
本章将会讲述控制程序主系统并驾驶飞机。这里假设读者已经熟悉飞行，也许是从其他模拟器里习得。如果你对飞行完全是新手，那么对你来说第 \ref{tutorials} 章的教程是更好的学习材料，可以学到如何在 \FlightGear{} 里飞行。
...
%%%%%%%%%%%%%%%
\ifchinese
取决于航空器的类型，你可能需要在飞行前起动发动机。下面的讲述是通用的方式。请查看航空器的帮助或者航空器教程来获取更多信息。

起动发动机以后，你需要检查是不是设置了\Index{驻留刹车}，如果已经设置可以按“B”键释放刹车。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
对活塞式飞机而言，磁电机点火系统用 “\{” 和 “\}” 键来控制。对大多数飞机而言，用 “s” 键来使用点火器。对有多台发动机的飞机而言你可以选择控制使用哪个发动机。使用 “\textasciitilde{}” 键选择所有发动机同时起动。大多数磁电机有四个档位——关断（OFF），左（LEFT），右（RIGHT）和双侧（BOTH）。因此要起动选择的发动机，按“\}”键三次，然后再长按“s”键。

注意，二战时代的战斗机，其发动机起动过程一般比较复杂。请查询相应航空器的帮助文档。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
\subsection{涡轮螺旋桨飞机}\index{发动机!起动!涡轮螺旋桨}
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
起动一台喷气式发动机显然更加复杂，而控制方法也与不同的航空器有关。
\fi
\iffalse
\IfLanguageName{english}{
...
下面的这些键位绑定\index{键位绑定!配置}并不是硬编码的，而是用户可调整的。查看和修改 \FlightGear{} 主目录下 \texttt{keyboard.xml}\index{keyboard.xml} 文件来配置键位设定。这是一份人类可读的 ASCII 文件。对初学来说，修改此文件并不明智，不过高级用户可以按照自己的想法修改键位绑定，比如适应其他模拟器的习惯。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
\subsection{航空器控制}\index{键盘控制!主要}
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
为了可以在飞行中完全控制飞机，你需要确保 \texttt{NumLock} 打开，且 \FlightGear{} 窗口在焦点位置。下面这些按键控制航空器的主要控制面。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
\centerline{表 3：\textit{航空器次要系统控制\index{键盘控制!航空器}}}
\fi
\iffalse
\IfLanguageName{english}{
...
有三种\Index{鼠标模式}：正常模式（默认），飞行控制模式和查看模式，你可以用 Tab 键在这些模式之间切换。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
在控制模式下，你通过移动鼠标来控制航空器，此模式使用一个十字作为鼠标指针。

在此模式下，移动鼠标向左或向右可控制副翼让航空器滚转。移动鼠标向前或向右控制升降舵可改变航空器的俯仰姿态。

按住左键可以改变控制行为，此时左右移动鼠标就可以控制方向舵。按住鼠标中键并前后移动鼠标就可以控制油门。

最后，鼠标滚轮则可以用来设置升降舵配平调整片。

若你没有游戏杆的时候使用此模式非常有用，可以提供比键盘更好的航空器控制。如果你打算经常使用鼠标控制航空器，建议打开自动协调选项，这样副翼和方向舵会一致控制。可以在启动模拟器时指定\\\texttt{-$ $-enable-auto-coordination} 选项来启用这个功能，或者在启动器里勾选自动协调（auto-coordination）选项。
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
在查看模式，你可以使用鼠标四处围观。此模式使用一个双向箭头作为鼠标指针图标。
...
\begin{itemize}
  ...
  \item \textbf{Sound Configuration} 配置各声音通道的音量，以及是否可以在飞机外听到。
    \item \textbf{Mouse Configuration} 配置鼠标的行为。
  \item \textbf{Joystick Configuration} 可以在模拟器里配置你的游戏杆，包括各种轴和按键映射。
  \item \textbf{Scenery Download} 配置是否使用 \TerraSync{} 特性自动从互联网下载地景。
  \item \textbf{Aircraft Center (Experimental)} 让你可以从模拟器界面下载和安装航空器，试验性的，并不完善。
  \item \textbf{Quit (Esc)} 退出\index{退出}此程序。
  \end{itemize}
\fi
\iffalse
\IfLanguageName{english}{
...
\ifchinese
\item \textbf{Autopilot}\index{自动驾驶} 此菜单仅在航空器默认配备自动驾驶仪时可用。一些飞机的自动驾驶是通过其仪表板来配置的，在这种情况下此菜单不可用。
\fi
\iffalse
\IfLanguageName{english}{
...
tramite l'apposito pannello.
 ...
 \begin{itemize}
\ifchinese
\item \textbf{Autopilot Settings (F11)} 配置航空器的自动驾驶。你可以设置自动驾驶的各种方式——从简单的保持机翼水平到沿 ILS 进近。
...
\begin{itemize}
  ...
  \item \textbf{Fuel and Payload} 可以设置燃油量和当前的航空器载重。只在一部分航空器上可用。
  ...
  \item \textbf{Random Failures} 配置随机的航空器系统和仪表失效。此设置为平均故障间隔时间（MTBF）或平均故障间隔周期（MCBF）。
  \item \textbf{System Failures} 配置随机的航空器系统失效，比如真空系统。
...
\ifchinese
\item \textbf{Debug}\index{调试} 此菜单包括了本指南范围以外的选项。
\fi
\iffalse
\IfLanguageName{english}{
...
%%%%%%%%%%%%%%%
 ...
 \centerline{\fbox{
\includegraphics[clip,width=14.0cm]{img/c172p-panel}
}}
...
\textbf{D}isplay，平视显示器）\index{平视显示器 Head Up Display}。HUD 一般在军用飞机和非常先进的喷气式飞机上能看到。然而 \FlightGear{} 可以在很多通用航空器上使用 HUD。要启用 HUD，只需按“h”键。

图7中的 \Index{HUD} 显示了飞机的主要飞行参数。中间你可以找到\Index{俯仰指示器}（以度表示），其上方是\Index{副翼指示器}而下方是\Index{方向舵指示器}。相应的左侧滑块是\Index{升降舵配平指示器}，旁边则是升降舵配平调整片指示器。而最下方则是一个简单的\Index{转弯指示器}。
...
\medskip
 ...
 \centerline{\fbox{
\includegraphics[clip,width=14.0cm]{img/HUD2}
}}
